...
class Node():
    ...
    def printSSFValue(self, prefix, allFeat):
        ...
        if self.__attributes:
            ...
            return ['\t'.join(x for x in returnValue)]
...
class Sentence():
        ...
        for line in self.text.split('\n'):
            ...
            if stripLine == "":
                ...
                if len(splitLine) > 1 and splitLine[1] == '((':
                    ...
                    if lastContext.__class__.__name__ != 'Sentence':
                        currentChunkNode.footer = line + '\n'
                        currentChunkNode.analyzeChunk()
                        lastContext = currentChunkNode.upper
                        currentChunkNode = lastContext
                    else:
                        self.errors.append('Encountered a chunk with missing chunk header ((')
                        self.probSent = True
                else:
                    if len(splitLine) < 3:
                        self.errors.append('Encountered a token line with less than 3 fields')
                        self.probSent = True
                    else:
                        currentNode = Node(line + '\n')
                        if lastContext.__class__.__name__ == 'Sentence':
                            self.errors.append('Encountered a token line with missing chunk header ((')
                            self.probSent = True
                            # chunk_address = line.split('\t')[0].split('.')[0]
                            # chunk_header = '\t'.join([chunk_address, '((', 'NP'])
                            # currentChunkNode = ChunkNode(chunk_header + '\n')
    ...
    def generateSentence(self):
        ...
        for nodeName in self.nodeList:
            nodeNameVal = nodeName.printValue()
            if not re.search('^NULL$', nodeNameVal, re.I):
                sentence.append(nodeNameVal)
...
def getTokenFeats(lineList):
    ...
    if len(lineList) > 1:
        ...
        returnErrors.append('Token is blank')
    multipleFeatRE = r'<fs.*?>'
    featRE = r'(?:\W*)(\S+)=([\'|\"])?([^ \t\n\r\f\v\'\"]*)[\'|\"]?(?:.*)'
    fsList = re.findall(multipleFeatRE, ' '.join(lineList))
    for x in lineList:
